...
class _AgregarClienteAlquilerScreenState
  ...
  void dispose() {
    ...
    _direccionCtrl.dispose(); _notaCtrl.dispose(); _inmuebleCtrl.dispose();
    _montoCtrl.dispose();
    super.dispose();
  }
    ...
    return Form(
      ...
      child: Column(
        children: [
          ...
          const Text('ALQUILER', style: TextStyle(color: Colors.white70, fontWeight: FontWeight.bold, fontSize: 12)),
          ...
          Container(
            ...
            child: Column(
              children: [
                ...
                TextFormField(controller: _telefonoCtrl, keyboardType: TextInputType.phone, inputFormatters: [TelefonoInputFormatter()], decoration: _deco('Teléfono', icon: Icons.call), validator: (v) => v!.isEmpty ? 'Obligatorio' : null),
                const SizedBox(height: 12),
                TextFormField(controller: _direccionCtrl, decoration: _deco('Dirección', icon: Icons.home_rounded)),
                const SizedBox(height: 12),
                TextFormField(controller: _inmuebleCtrl, decoration: _deco('Inmueble / Propiedad', icon: Icons.house_rounded), validator: (v) => v!.isEmpty ? 'Obligatorio' : null),
                const SizedBox(height: 12),
                ...
                TextFormField(
                  ...
                  maxLines: 2,
                  decoration: _deco('Nota o detalles', icon: Icons.edit_note),
                ),
  ...
  Widget _moraSection() {
    return Container(
      ...
      decoration: BoxDecoration(color: const Color(0xFFF7F8FA), borderRadius: BorderRadius.circular(14), border: Border.all(color: const Color(0xFFE5E7EB))),
      child: Column(
        children: [
          Row(
            children: [
              const Expanded(child: Text('Mora por atraso', style: TextStyle(fontWeight: FontWeight.w700))),
              Switch.adaptive(value: _moraEnabled, activeColor: const Color(0xFF2563EB), onChanged: (v) => setState(() => _moraEnabled = v)),
            ],
          ),
          if (_moraEnabled) ...[
            ...
            Row(
              children: [
                ChoiceChip(label: const Text('Porcentaje'), selected: _moraTipo == 'porcentaje', onSelected: (_) => setState(() => _moraTipo = 'porcentaje')),
                const SizedBox(width: 8),
                ChoiceChip(label: const Text('Monto fijo'), selected: _moraTipo == 'fijo', onSelected: (_) => setState(() => _moraTipo = 'fijo')),
              ],
            ),
            const SizedBox(height: 8),
            TextFormField(
              ...
              decoration: _deco(_moraTipo == 'porcentaje' ? 'Mora (%)' : 'Mora (\$)'),
              onChanged: (v) => _moraValor = double.tryParse(v.replaceAll(',', '.')) ?? 0,
            ),
          ],
        ],
      ),
  ...
  Widget _fechaSection() {
    return InkWell(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(color: const Color(0xFFF7F8FA), borderRadius: BorderRadius.circular(14), border: Border.all(color: _proximaFecha == null ? Colors.red : const Color(0xFFE5E7EB))),
        child: Row(
          children: [
            Icon(Icons.date_range, color: _proximaFecha == null ? Colors.red : Colors.blue),
            const SizedBox(width: 10),
            Text(_proximaFecha == null ? 'Elegir Próxima Fecha' : 'Pago: ${_fmtFecha(_proximaFecha!)}', style: TextStyle(fontWeight: FontWeight.bold, color: _proximaFecha == null ? Colors.red : Colors.black87)),
          ],
        ),
      ),
    );
  ...
  Widget _btnGuardar() {
    return SizedBox(
      ...
      child: ElevatedButton(
        ...
        child: _guardando ? const CircularProgressIndicator(color: Colors.white) : const Text('GUARDAR ALQUILER', style: TextStyle(fontWeight: FontWeight.bold)),
      ),
    );
  }
  ...
  Future<void> _guardar() async {
    ...
    final monto = int.tryParse(_montoCtrl.text.replaceAll(RegExp(r'[^0-9]'), '')) ?? 0;
    ...
    final data = {
      ...
      'mora': _moraEnabled ? {'tipo': _moraTipo, 'valor': _moraValor} : null,
    };
    ...
    try {
      ...
      if (mounted) Navigator.pushAndRemoveUntil(context, MaterialPageRoute(builder: (_) => const ClientesScreen(initFiltro: 'alquiler')), (r) => false);
    } catch (e) {
