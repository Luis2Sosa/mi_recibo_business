// lib/clientes/productos_screen.dart
...
class ProductosScreen extends StatelessWidget {
  ...
  bool _esTextoAlquiler(String p) {
    p = p.toLowerCase();
  ...
  int _compareClientes(Cliente a, Cliente b) {
    ...
    if (sa != sb) return sa ? 1 : -1; // saldados van al final
    ...
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (_, snap) {
        ...
        final lista = docs.map((d) {
          ...
          final codigoVisible =
          (codigoGuardado != null && codigoGuardado.isNotEmpty)
              ? codigoGuardado
              : _codigoDesdeId(d.id);
        ...
        final filtered = lista.where((c) {
          ...
          final isProducto = prod.isNotEmpty && !_esTextoAlquiler(prod);
          final match = c.codigo.toLowerCase().contains(q) ||
              c.nombreCompleto.toLowerCase().contains(q) ||
              c.telefono.contains(q);
          return isProducto && match;
        }).toList()
          ..sort(_compareClientes);

        if (filtered.isEmpty) {
          return const Center(
            child: Text('No hay productos', style: TextStyle(color: Colors.white)),
          );
        }
