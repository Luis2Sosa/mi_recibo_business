...
class _AgregarClientePrestamoScreenState
        ...
        child: Column(
          children: [
            ...
            TextFormField(
              ...
              maxLines: 2,
              decoration: _deco('Nota adicional', icon: Icons.edit_note),
            ),
            ...
            TextFormField(
              ...
              decoration:
              _deco('Monto Prestado (\$)', icon: Icons.monetization_on),
              validator: (v) => v!.isEmpty ? 'Escribe el monto' : null,
            ),
            const SizedBox(height: 12),
        ...
        child: Row(
          children: [
            Icon(Icons.calendar_today,
                size: 18,
                color: _proximaFecha == null ? Colors.red : Colors.blue),
            const SizedBox(width: 10),
            Text(
              _proximaFecha == null
                  ? 'Elegir Próximo Pago'
  ...
  Future<void> _guardar() async {
    ...
    final data = {
      ...
      'periodo': _periodo,
      'proximaFecha':
      Timestamp.fromDate(_atNoon(_proximaFecha!)),
    ...
    try {
      if (_isEdit) {
        ...
        await metricRef.set(
            {
              ...
              FieldValue.increment((capital - capAnterior).toDouble())
            },
        ...
        await metricRef.set(
            {
              'totalCapitalPrestado':
              FieldValue.increment(capital.toDouble())
            },
            SetOptions(merge: true));
      }
      if (mounted) {
        Navigator.pushAndRemoveUntil(
          ...
          MaterialPageRoute(
              builder: (_) =>
              const ClientesScreen(initFiltro: 'prestamos')),
              (r) => false,
        );
      }
    } catch (e) {
