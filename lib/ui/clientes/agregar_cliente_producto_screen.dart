...
class _AgregarClienteProductoScreenState
  ...
  void _agregarProducto() {
    setState(() {
      _productos.add({
        'nombre': TextEditingController(text: _productos.isEmpty ? (widget.initProducto ?? '') : ''),
              ...
              children: [
                Text(
                  ...
                  style: GoogleFonts.playfairDisplay(
                    textStyle: const TextStyle(color: Colors.white, fontSize: 28, fontStyle: FontStyle.italic, fontWeight: FontWeight.w600),
                  ),
                ),
                const SizedBox(height: 4),
                const Text('Productos / Fiados', style: TextStyle(color: Colors.white70, fontWeight: FontWeight.bold, fontSize: 14)),
              ],
            ),
          ),
    ...
    return _tarjeta(
      child: Column(
        children: [
          Row(
            children: [
              Expanded(child: TextFormField(controller: _nombreCtrl, textCapitalization: TextCapitalization.sentences, decoration: _deco('Nombre', icon: Icons.person), validator: (v) => (v == null || v.isEmpty) ? 'Obligatorio' : null)),
              const SizedBox(width: 12),
              Expanded(child: TextFormField(controller: _apellidoCtrl, textCapitalization: TextCapitalization.sentences, decoration: _deco('Apellido', icon: Icons.badge), validator: (v) => (v == null || v.isEmpty) ? 'Obligatorio' : null)),
            ],
          ),
          const SizedBox(height: 12),
          TextFormField(controller: _telefonoCtrl, keyboardType: TextInputType.phone, inputFormatters: [TelefonoInputFormatter()], decoration: _deco('Teléfono', icon: Icons.call), validator: (v) => (v == null || v.isEmpty) ? 'Obligatorio' : null),
          const SizedBox(height: 12),
          TextFormField(controller: _direccionCtrl, textCapitalization: TextCapitalization.sentences, decoration: _deco('Dirección (opcional)', icon: Icons.home)),
          const SizedBox(height: 12),
          TextFormField(controller: _notaCtrl, textCapitalization: TextCapitalization.sentences, decoration: _deco('Nota (opcional)', icon: Icons.note_alt_outlined), maxLines: 2),
        ],
      ),
    );
  }
  ...
  Widget _listaProductos() {
    return _tarjeta(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text('Productos o Servicios', style: TextStyle(fontWeight: FontWeight.bold, color: Color(0xFF1E293B), fontSize: 16)),
          const SizedBox(height: 12),
            ...
            children: [
              Expanded(
                child: ElevatedButton.icon(
                  ...
                  style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF2563EB), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(25))),
                ),
              ),
              if (_productos.length > 1) ...[
                ...
                Expanded(
                  child: ElevatedButton.icon(
                    onPressed: () => setState(() { _productos.removeLast(); _calcularTotales(); }),
                    icon: const Icon(Icons.remove_rounded, color: Colors.white),
                    label: const Text('Quitar', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w600)),
  ...
  Widget _productoCard(int index) {
    ...
    return Container(
      ...
      child: Column(
        children: [
          TextFormField(controller: prod['nombre'], decoration: _deco('Producto', icon: Icons.local_offer), validator: (v) => (v == null || v.isEmpty) ? 'Obligatorio' : null),
          const SizedBox(height: 10),
          Row(
            children: [
              Expanded(child: TextFormField(controller: prod['precioBase'], keyboardType: TextInputType.number, decoration: _deco('Costo'), onChanged: (_) => _calcularTotales())),
              const SizedBox(width: 10),
              Expanded(child: TextFormField(controller: prod['precioCliente'], keyboardType: TextInputType.number, decoration: _deco('Venta'), onChanged: (_) => _calcularTotales())),
            ],
          ),
        ],
      ),
  ...
  Widget _resumenTotales() {
    ...
    return _tarjeta(
      child: Column(
        children: [
          _iconLabel(Icons.trending_up_rounded, 'Ganancia estimada', 'RD\$${_gananciaTotal.toStringAsFixed(0)}', color: Colors.blue),
          const SizedBox(height: 10),
          _iconLabel(Icons.account_balance_wallet_rounded, 'Monto total', 'RD\$${_montoTotal.toStringAsFixed(0)}', color: Colors.cyan),
          const SizedBox(height: 10),
          TextFormField(controller: _pagoInicialCtrl, keyboardType: TextInputType.number, decoration: _deco('Pago inicial', icon: Icons.payments_outlined), onChanged: (_) => setState(() {})),
          const SizedBox(height: 12),
          Container(
            width: double.infinity, padding: const EdgeInsets.all(14),
            decoration: BoxDecoration(color: const Color(0xFFF0F9FF), borderRadius: BorderRadius.circular(14), border: Border.all(color: const Color(0xFFBAE6FD))),
            child: Text('Monto restante: RD\$${montoRestante.toStringAsFixed(0)}', style: const TextStyle(color: Color(0xFF0369A1), fontWeight: FontWeight.bold, fontSize: 16)),
          ),
        ],
      ),
    );
  ...
  Widget _iconLabel(IconData icon, String label, String value, {Color color = Colors.black}) {
    return Row(children: [
  ...
  Widget _fechaSection() {
    return _tarjeta(
      ...
      child: Row(
        children: [
          Expanded(child: Text(_proximaFecha == null ? 'Selecciona fecha de pago' : 'Fecha: ${_proximaFecha!.day}/${_proximaFecha!.month}/${_proximaFecha!.year}', style: TextStyle(fontWeight: FontWeight.w600, color: _proximaFecha == null ? Colors.red : Colors.black87))),
          TextButton.icon(icon: const Icon(Icons.calendar_today_outlined), label: const Text('Elegir'), onPressed: () async {
  ...
  Widget _tarjeta({required Widget child, Color? colorFondo}) {
    return Container(width: double.infinity, padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: colorFondo ?? Colors.white, borderRadius: BorderRadius.circular(24)), child: child);
  }
  ...
  Widget _botonGuardar() {
    return SizedBox(
      ...
      child: ElevatedButton(
        style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF2458D6), foregroundColor: Colors.white, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(30))),
        onPressed: _guardando ? null : _guardar,
        child: _guardando ? const CircularProgressIndicator(color: Colors.white) : const Text('GUARDAR', style: TextStyle(fontWeight: FontWeight.bold)),
      ),
    );
  }
  ...
  Future<void> _guardar() async {
    ...
    final clientesRef = db.collection('prestamistas').doc(uid).collection('clientes');

    final pagoInicial = double.tryParse(_pagoInicialCtrl.text) ?? 0;
    final saldoActual = (_montoTotal - pagoInicial).clamp(0, 999999999);

    double costoTotal = 0;
    for (final p in _productos) costoTotal += (double.tryParse(p['precioBase']!.text) ?? 0);
    ...
    final data = {
      ...
      'producto': _productos.isNotEmpty ? _productos.first['nombre']!.text.trim() : 'Producto',
      ...
      'proximaFecha': Timestamp.fromDate(DateTime(_proximaFecha!.year, _proximaFecha!.month, _proximaFecha!.day, 12)),
      ...
      if (pagoInicial > 0) {
        ...
        Navigator.of(context).pushReplacement(
          MaterialPageRoute(
            builder: (_) => ReciboScreen(
              ...
              numeroRecibo: "INI-${DateTime.now().millisecondsSinceEpoch % 10000}",
              fecha: DateTime.now(),
              capitalInicial: costoTotal.toInt(),
              pagoInteres: 0,
              pagoCapital: pagoInicial.toInt(),
        ...
        if (mounted) {
          Navigator.of(context).pushAndRemoveUntil(
              MaterialPageRoute(builder: (_) => const ClientesScreen(initFiltro: 'productos')),
                  (r) => false
          );
        }
      }
