...
class _GananciasScreenState extends State<GananciasScreen> {
  ...
  // =========================================================
  // NUEVA VERSIÓN: KPI separados por tipo de cliente
  // =========================================================
  Future<_GananciasResumen> _cargarGananciasGlobal() async {
    ...
    // === NUEVOS: alquiler ===
    int totalPendienteAlquiler = 0;
    int totalRecuperadoAlquiler = 0;
    ...
    for (final c in cs.docs) {
      ...
      if (tipo == 'alquiler') {
        if (saldoActual > 0) totalPendienteAlquiler += saldoActual;
      } else {
      ...
      for (final p in pagos.docs) {
        ...
        (m['totalPagado'] ?? (pagoInteres + pagoCapital)) as int;

        if (tipo == 'alquiler') {
          totalRecuperadoAlquiler += totalPagado;
        } else {
      ...
      // --- Circulando solo aplica a préstamos y productos ---
      if (tipo != 'alquiler') {
    ...
    // === Totales finales ===
    final int totalPendienteFinal = totalPendiente + totalPendienteAlquiler;
    final int totalRecuperadoFinal = totalRecuperado + totalRecuperadoAlquiler;
    final double recuperacionPct = (totalPrestado <= 0)
        ? 0.0
        : (totalRecuperadoFinal / totalPrestado * 100.0);

    return _GananciasResumen(
      pendiente: totalPendienteFinal,
      circulando: totalCirculando,
      recuperado: totalRecuperadoFinal,
                            ...
                            children: [
                              ...
                              Container(
                                ...
                                decoration: BoxDecoration(
                                  ...
                                  gradient: LinearGradient(
                                      colors: [AppTheme.gradBottom, AppTheme.gradTop]),
                                  boxShadow: [
                                    BoxShadow(
                            ...
                            child: Row(
                              ...
                              children: [
                                Icon(Icons.trending_up_rounded,
                                    color: AppTheme.gradBottom.withOpacity(.95),
                                    size: 28),
                      ...
                      child: ElevatedButton.icon(
                        onPressed: () {
                          Navigator.push(
                            ...
                            MaterialPageRoute(
                              builder: (_) =>
                                  GananciaClientesScreen(docPrest: widget.docPrest),
                            ),
                          );
                        },
            ...
            children: [
              Container(
                ...
                decoration: BoxDecoration(
                  ...
                  border: Border.all(
                      color: Colors.white.withOpacity(.45), width: 1.3),
                ),
    ...
    return Row(
      children: [
        IconButton(
          ...
          style: ButtonStyle(
            backgroundColor:
            MaterialStateProperty.all(AppTheme.gradTop.withOpacity(.9)),
